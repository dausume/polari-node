...
# ==============================================================================
# Deploy-time security: an existing file with REAL (non-placeholder) values
# is KEPT unless rotation is asked for (POLARI_ROTATE_KC=yes) — re-running
# setup for config changes must not silently rotate secrets. Files carrying
# placeholder/default secret values are never kept (fail closed). Every
# write is stamped in the security ledger; material older than
# SEC_STALE_DAYS gets a rotation suggestion, never an auto-rotate.
source "$SCRIPT_DIR/security-ledger.sh"
...
if [ -f "$KC_ADMIN_ENV_FILE" ] && [ "$(sec_placeholders "$KC_ADMIN_ENV_FILE")" -eq 0 ] \
   && [ "${POLARI_ROTATE_KC:-no}" != "yes" ]; then
    KC_AGE=$(ledger_age_days kc-admin-env "$KC_ADMIN_ENV_FILE")
    echo -e "  Kept existing credentials: $KC_ADMIN_ENV_FILE (last updated ${KC_AGE:-?}d ago)"
    if sec_is_stale kc-admin-env "$KC_ADMIN_ENV_FILE"; then
        echo -e "  ${YELLOW}STALE: last rotation was ${KC_AGE}d ago (> ${SEC_STALE_DAYS}d).${NC}"
        echo -e "  ${YELLOW}Rotate smoothly with: pol security rotate staging${NC}"
    fi
else
...
EOF
    ...
    ledger_stamp kc-admin-env "$KC_ADMIN_ENV_FILE" staging-setup.sh
    echo -e "  Generated: ${GREEN}$KC_ADMIN_ENV_FILE${NC}"
    if [ "$KC_ADMIN_PASS" = "admin" ]; then
        echo -e "  ${YELLOW}WARNING: Keycloak admin password is the dev default 'admin' —${NC}"
        echo -e "  ${YELLOW}acceptable on LAN staging only, NEVER internet-facing.${NC}"
        echo -e "  ${YELLOW}Set POLARI_KC_ADMIN_PASS (or use prod-setup.sh) for a real one.${NC}"
    fi
fi

# ==============================================================================
# Generate prf-mariadb/mariadb.env IF MISSING. DB passwords are baked into the
# MariaDB volume at first init — never overwrite an existing file, or Keycloak
# and the backend would desync from the volume's real passwords.
...
if [ -f "$MARIADB_ENV_FILE" ]; then
    ...
    if [ "$(sec_placeholders "$MARIADB_ENV_FILE")" -gt 0 ]; then
        echo -e "  ${YELLOW}WARNING: it carries placeholder/default DB passwords, and those are${NC}"
        echo -e "  ${YELLOW}baked into the DB volume — rotating needs a FRESH prf-mariadb volume${NC}"
        echo -e "  ${YELLOW}(delete this file + the volume, then re-run setup).${NC}"
    fi
else
    cat > "$MARIADB_ENV_FILE" << EOF
# ==============================================================================
# POLARI RESEARCH FRAMEWORK - MARIADB CREDENTIALS
# Generated by staging-setup.sh — NOT tracked in git.
...
EOF
    ...
    ledger_stamp mariadb-env "$MARIADB_ENV_FILE" staging-setup.sh
