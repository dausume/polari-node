...
# Deploy-time security: detect existing material first. Real credentials are
# kept by default when fresh; older than SEC_STALE_DAYS gets a rotation
# recommendation; placeholder/default values are NEVER kept (fail closed).
# POLARI_ROTATE_KC=yes|no answers the prompt for non-interactive runs.
...
ROTATE_KC=yes
if [ -f "$KC_ADMIN_ENV_FILE" ]; then
    if [ "$(sec_placeholders "$KC_ADMIN_ENV_FILE")" -gt 0 ]; then
        echo -e "  ${RED}Existing $KC_ADMIN_ENV_FILE carries placeholder/default values —${NC}"
        echo -e "  ${RED}rotating unconditionally (placeholders are never deployable).${NC}"
    elif [ "${POLARI_ROTATE_KC:-}" = "yes" ]; then
        echo -e "  Rotating Keycloak credentials on request (POLARI_ROTATE_KC=yes)."
    elif [ "${POLARI_ROTATE_KC:-}" = "no" ]; then
        ROTATE_KC=no
    else
        KC_AGE=$(ledger_age_days kc-admin-env "$KC_ADMIN_ENV_FILE")
        if sec_is_stale kc-admin-env "$KC_ADMIN_ENV_FILE"; then
            echo -e "  ${YELLOW}Existing Keycloak credentials are ${KC_AGE}d old (> ${SEC_STALE_DAYS}d) —${NC}"
            echo -e "  ${YELLOW}rotation recommended.${NC}"
            read -p "  Rotate them now? (Y/n): " KC_ANSWER
            [[ "$KC_ANSWER" =~ ^[Nn] ]] && ROTATE_KC=no
        else
            echo -e "  Existing Keycloak credentials found (last updated ${KC_AGE:-?}d ago — fresh)."
            read -p "  Keep them? (Y/n): " KC_ANSWER
            [[ "$KC_ANSWER" =~ ^[Nn] ]] || ROTATE_KC=no
        fi
    fi
fi

if [ "$ROTATE_KC" = "yes" ]; then
...
EOF
    ...
    ledger_stamp kc-admin-env "$KC_ADMIN_ENV_FILE" prod-setup.sh
    ...
    echo -e "  ${YELLOW}Roll it out KEYCLOAK FIRST, then the backend (pol security rotate prod${NC}"
    echo -e "  ${YELLOW}does the ordered restart for you on a running deployment).${NC}"
else
    echo -e "  Kept existing credentials: $KC_ADMIN_ENV_FILE"
fi
...
EOF
    ...
    ledger_stamp mariadb-env "$MARIADB_ENV_FILE" prod-setup.sh
    echo -e "  Generated: ${GREEN}$MARIADB_ENV_FILE${NC}"
fi
if [ "$(sec_placeholders "$MARIADB_ENV_FILE")" -gt 0 ]; then
    echo -e "  ${RED}WARNING: $MARIADB_ENV_FILE carries placeholder/default DB passwords.${NC}"
    echo -e "  ${RED}They are volume-coupled — rotate by deleting the file AND the prf-mariadb${NC}"
    echo -e "  ${RED}volume, then re-running this setup (fresh deployments never hit this).${NC}"
fi
